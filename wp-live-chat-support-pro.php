        ...
        if ($_POST['action'] == "wplc_start_chat") {
            ...
            if (isset($_POST['cid'])) {
                if ($_POST['name'] && $_POST['email']) {
                    echo wplc_initiate_chat(sanitize_text_field($_POST['name']), sanitize_email($_POST['email']), sanitize_text_field($_POST['cid']), $_POST['wplcsession']); // echo the chat session id
                } else {
                    echo "error2";
                }
            } else {
                if ($_POST['name'] && $_POST['email']) {
                    echo wplc_initiate_chat(sanitize_text_field($_POST['name']), sanitize_email($_POST['email']), null, $_POST['wplcsession']); // echo the chat session id
                } else {
                    echo "error2";
                }
            }
...
function wplc_pro_admin_menu_layout_display() {
    ...
    if (!isset($_GET['action'])) {
        ...
        <?php $agent_id = wplc_ma_check_if_user_is_agent(); ?>      
        ...
        if (isset($_GET['aid'])) {
            if(get_option('wplc_use_external_server')){
                wplc_ma_update_agent_id_ex(sanitize_text_field($_GET['cid']), sanitize_text_field($_GET['aid']));
            } else {
...
function wplc_admin_pro_request_chat($cid) {
    if(get_option('wplc_use_external_server')){
    	wplc_change_chat_status_ex(sanitize_text_field($_GET['cid']), 6, get_option('siteurl'));
        echo wplc_pro_draw_chat_area_ex($cid); 
    } else {
...
function wplc_push_js_to_front_pro() {
  ...
  /* do not show if pro is outdated */
    ...
    if (isset($wplc_pro_settings['wplc_pro_offline3'])) { wp_localize_script('wplc-user-script', 'wplc_offline_msg', stripslashes($wplc_pro_settings['wplc_pro_offline2'])); } else { wp_localize_script('wplc-user-script', 'wplc_offline_msg3',__("Sending message...", "wplivechat")); }
    if (isset($wplc_pro_settings['wplc_pro_offline3'])) { wp_localize_script('wplc-user-script', 'wplc_offline_msg3',stripslashes($wplc_pro_settings['wplc_pro_offline3'])); } else { wp_localize_script('wplc-user-script', 'wplc_offline_msg3',__("Thank you for your message. We will be in contact soon.", "wplivechat")); }
...
function wplc_admin_menu_pro() {
    ...
    /* offline messages added to Pro version in 5.1.00 */
    if (intval(str_replace(".","",$wplc_version)) < 5100) {
    ...
    if (function_exists("wplc_extensions_menu")) {
...
/* pro hook control */
add_action("wplc_hook_output_box_header","wplc_hook_control_pages_that_can_view_chat");


function wplc_hook_control_pages_that_can_view_chat() {
